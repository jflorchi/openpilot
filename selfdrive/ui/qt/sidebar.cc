...
void Sidebar::updateState(const UIState &s) {
  ...
  float amps = deviceState.getBatteryCurrent() / 1000.0;
  amps = roundf(amps * 100) / 100;
  ...
  ItemStatus batteryStatus = {"CURRENT\n" + QString("%1mA").arg(amps), tempColor2};
  setProperty("connectStatus", QVariant::fromValue(batteryStatus));
...
//  setProperty("connectStatus", QVariant::fromValue(connectStatus));
  ...
  setProperty("tempStatus", QVariant::fromValue(ItemStatus{"TEMP\n" + QString("%1°C").arg((int)deviceState.getAmbientTempC()), tempColor}));
